    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="space-y-6">
        {categoryPosts.map((post) => (
          <article
            key={post.id}
            className="bg-card rounded-lg shadow-md hover:shadow-lg transition-shadow border border-border"
          >
            <Link
      ...
      {categoryPosts.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-card-foreground mb-2">아직 게시글이 없습니다</h3>
          <p className="text-muted-foreground">곧 흥미로운 {category.name} 관련 글들을 만나보실 수 있습니다.</p>
        </div>
      )}
    </div>
  );
